# what - Reversing Writeup (Codex)

## Goal
Recover the flag from the `what` binary and the embedded bytecode-like program.

## High-level Observation
The binary interprets a string of opcodes over user input. Each input character is:
1. Loaded on `?`.
2. Transformed by a sequence of operations (`W`, `H`, `A`) using a repeating key.
3. Compared to a constant at `T`.

So each `T` compares a derived value against a known `solution[i]`. This is a straight reversible transformation.

## VM Semantics from Source
From `src/what.c`:
- Key bytes: `"WHAT"` (cycle length 4).
- Operations:
  - `W`: `in ^= key[idx]`
  - `H`: `in += key[idx]`
  - `A`: `in *= key[idx]`
  - `T`: check `in == solution[i]`

The key index advances on every `W/H/A`, and **does not reset** between input characters.

## Inversion Strategy
For each segment of the program between consecutive `?` markers:
1. Let `solution[i]` be the target value checked by `T`.
2. Walk the opcodes in reverse order.
3. Apply inverse operations:
   - `W`: xor (self-inverse)
   - `H`: subtract
   - `A`: integer divide
4. Track the key index across segments so it matches the forward execution.

## Generated Solver (Full)
Saved as `generated_solve.py` in this folder.

```py
a = [3924, 403840174523760, 10708771522428, 353237081188, 5127893933, 149960298, 3248700, 32837, 2179047144047076, 164451697868777, 600811115256, 2527683126, 150275532, 2860847, 3449, 1828556732620887, 24597419141883, 61783876357, 1197077196, 17200793, 89635, 4077, 2912224039305411, 72598267718928, 59639151351, 2258366787, 228691224, 3892891, 65943, 833426982984960, 49488936457716, 97987125600, 1573266557, 45132237, 2023779, 17452, 4748452348696368, 23263942584024, 509490137403, 382313468, 69424894, 1878085, 44548, 12045994937322663, 23529222662400, 86342566505, 2231651489, 69418347, 2608956, 12921, 4973645091643620, 32311478428563, 280472233860, 1584955224, 323581956, 2493272, 39508, 2917682891468458, 21280518312514, 106708300920, 1402259127]

program = "?WAWWHT?WAAWWAHHWAWAAAT?WAAHAAHHAAT?WHAAAHAHAWWHT?WHAAHHAHAWHT?WWHHWWHAAAHHWHT?WHHHHHHHAAT?WHHHHHHWWAHHT?WHAAAHAHAWHHHHHAAHT?WHHWHHAHHAAAHAAHHHT?WHHHAHWHHHAHHHAHAAT?WAAHHAHHHAHHWHHHHHT?WHHHHAHHAHAHWHHHHHT?WHHHHHHHWAHHAHHHHHT?WAWT?WHAAAAAAAWT?WHAAHAAAWAWWT?WAAAHAWAWHHT?WAAAHHHHAT?WAHHWHAHAHT?WAHHHHWWHWHAT?WAHWHHHWHHHT?WAHHAAAHHAAHHAHHT?WHHHAHWWHAHAHAWHHAAT?WAHWHHHWAAHHHWAHHHAWT?WAHHHHHAAHHHWHAHHT?WHHHHHAHHAHHHHHAT?WHHHHHHWWHAHWHHHAHHT?WHHHHHHWHHWHWHWHHHAHT?WAAWAAAAAT?WHAAAAAWWAT?WAWWHWWHAAAAT?WAAAAWWHHHWT?WHAHHAAHWT?WHWHWAHHAHT?WHAHHWWWHWHHT?WHHAHHHHAAAWHAAWAWT?WWAWHAHHHAHHAWHAAHT?WHHAHHHHWAAHAWHHAWT?WAHHAHWAHHWHHAHWHHT?WHAHHHHWHHAWHHHWAHT?WWHWAHHHHHHHAHHHHWT?WHHWWHHWHAHHHHHHHHT?WHWHHHHHAAHWAHHHHAAHAHWHAT?WAAAAAAT?WWAAWHAWAWAT?WAAAWAHWHT?WHAHWAHAWWT?WHHHHAAT?WWHAHHHHWWWT?WHHWAWAAAHAHAHHAT?WHAAHHAHAAHAHHT?WWAHHHHHAHHHAAAT?WAHAHHHWHHAHHHWWAT?WHHHHHAWHAHHHWAHT?WHHHHHAHAHHHHHT?WHHHWHHAHHHHHHHT?WHHAHHHWAHAHAWHHAHAAHHHWT?WHAHAHWHHWHAHAAHHHHWHWHAHT?WAAAWAAT?WAAAAHT!"

instrs = program[:-1].split("?")[1:]
instrs = [x for x in instrs if x != "T"]

key = [ord(x) for x in "WHAT"]
out = []
total_idx = 0

for i, solution in enumerate(a):
    idx = (total_idx + len(instrs[i]) - 1) % 4
    total_idx += len(instrs[i]) - 1
    for c in reversed(instrs[i]):
        if c == "W":
            solution ^= key[idx]
        elif c == "H":
            solution -= key[idx]
        elif c == "A":
            solution //= key[idx]

        idx -= 1
        if idx == -1:
            idx = 3
    out.append(chr(solution))

print("".join(out))
```

## Docker Validation
Executed in a clean container to verify both the original and generated solvers:

```bash
./verify.sh
```

Output:

```
OK: solve/what_solve.py
OK: generated_solve.py
All verification checks passed.
```

## Reproducible Verification
Included for collaborators:
- `Dockerfile.verify` builds a minimal Python environment.
- `verify.sh` runs both solvers and checks for the expected flag.

## Writeup Process (Reusable)
Use this sequence for future challenges to keep writeups reproducible:

1. Collect the challenge files, official solve script, and any existing writeups.
2. Reverse the binary and summarize the core insight.
3. Create a standalone `generated_solve.py` that prints the flag deterministically.
4. Add `Dockerfile.verify` and `verify.sh` to standardize validation.
5. Run `./verify.sh` and record the command and output in the writeup.
